  ...
  if [ -n "$BREW_PG_BIN" ]; then
    if "$BREW_PG_BIN/pg_isready" -q 2>/dev/null; then
      ...
      if "$BREW_PG_BIN/pg_isready" -q 2>/dev/null; then
        ...
        if ! "$BREW_PG_BIN/psql" -U postgres -lqt 2>/dev/null | cut -d\| -f1 | grep -qw construction_db; then
          info "Creating construction_db…"
          "$BREW_PG_BIN/createdb" -U postgres construction_db 2>/dev/null || \
            "$BREW_PG_BIN/psql" -d postgres -c "CREATE DATABASE construction_db OWNER postgres;" 2>/dev/null || true
        fi
      fi
    fi
  fi
