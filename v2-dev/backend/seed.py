"""Multi-tenant seed script. Creates 5 tenants in master DB and seeds each tenant DB."""
...
from app.db.session import MasterSessionLocal, get_tenant_engine
...
def seed_tenant(db, admin_email: str, tenant_name: str) -> None:
    if db.query(Project).count() > 0:
        print(f"  Demo data already exists for {tenant_name}. Skipping.")
        return
    ...
    projects = [
        Project(
            name=f"Villa Marrakech — {tenant_name}",
            ...
            description=f"Villa R+2 avec piscine, lotissement privé, finitions premium. ({tenant_name})",
            created_by=admin.id,
        ),
        Project(
            name=f"Immeuble R+4 Casablanca — {tenant_name}",
            client_type=ClientType.private,
            location="Casablanca",
            city="Casablanca",
            ...
            description=f"Immeuble résidentiel R+4 avec sous-sol et 20 appartements. ({tenant_name})",
            created_by=admin.id,
        ),
        Project(
            name=f"Résidence Al Baraka Temara — {tenant_name}",
            ...
            description=f"Résidence sociale publique, phase de préparation et montage budgétaire. ({tenant_name})",
            created_by=admin.id,
        ),
    ]
    db.add_all(projects)
    ...
    db.commit()
    print(f"  Seeded {tenant_name}: admin={admin_email}, projects=3, employees=11")
...
def seed():
    ...
    try:
        tenants = [
            {"id": 1, "name": "Promoteur Atlas", "slug": "atlas"},
            {"id": 2, "name": "BTP Horizon", "slug": "horizon"},
            {"id": 3, "name": "ConstructPlus Maroc", "slug": "constructplus"},
            {"id": 4, "name": "Génie Civil Pro", "slug": "geniecivil"},
            {"id": 5, "name": "Immobilière Nord", "slug": "nord"},
        ]

        for t in tenants:
            existing = master_db.query(Tenant).filter(Tenant.id == t["id"]).first()
            if not existing:
                tenant = Tenant(id=t["id"], name=t["name"], slug=t["slug"])
                master_db.add(tenant)
        master_db.commit()
        print("Master DB seeded with 5 tenants")

        for i, t in enumerate(tenants, start=1):
            engine = get_tenant_engine(i)
            TenantSessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
            tenant_db = TenantSessionLocal()
            try:
                print(f"\nSeeding tenant {i}/{len(tenants)} ({t['name']}) ...")
                seed_tenant(tenant_db, f"admin{i}@example.com", t["name"])
            except Exception as exc:
                tenant_db.rollback()
                print(f"  Seed error for {t['name']}: {exc}")
                raise
            finally:
                tenant_db.close()

        print("\n" + "=" * 50)
        print("Seed completed for all 5 tenants")
        print("=" * 50)
        for i, t in enumerate(tenants, start=1):
            print(f"  {t['name']}: admin{i}@example.com / password123")
        print("=" * 50)
    except Exception as exc:
        master_db.rollback()
        print(f"Master seed error: {exc}")
        raise
    finally:
        master_db.close()
